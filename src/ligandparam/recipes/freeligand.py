...
class FreeLigand(Recipe):
    ...
    def setup(self):
        ...
        self.stages = [
            ...
            StageLazyResp("Resp", main_input=lowtheory_minimization_gaussian_log, cwd=self.cwd, out_mol2=resp_mol2_low,
                          **self.kwargs),
            ...
            StageGaussiantoMol2("GrabGaussianCharge", main_input=hightheory_minimization_gaussian_log, cwd=self.cwd,
                                nproc=self.nproc, mem=self.mem, gaussian_root=self.gaussian_root,
                                gauss_exedir=self.gauss_exedir, gaussian_binary=self.gaussian_binary,
                                gaussian_scratch=self.gaussian_scratch, net_charge=self.net_charge, theory=self.theory,
                                force_gaussian_rerun=self.force_gaussian_rerun, template_mol2=initial_mol2,
                                out_mol2=resp_mol2_high, **self.kwargs),
            StageGaussianRotation("Rotate", main_input=resp_mol2_high, cwd=self.cwd, nproc=self.nproc, mem=self.mem,
                                  gaussian_root=self.gaussian_root, gauss_exedir=self.gauss_exedir,
                                  gaussian_binary=self.gaussian_binary, gaussian_scratch=self.gaussian_scratch,
                                  net_charge=self.net_charge, theory=self.theory,
                                  force_gaussian_rerun=self.force_gaussian_rerun, out_gaussian_label=rotation_label,
                                  alpha=[0, 30, 60, 90, 120, 150, 180], beta=[0, 30, 60, 90], gamma=[0], **self.kwargs),
            # We know that the gaussian stages work in a "gaussianCalcs" directory. Quite hacky.
            StageMultiRespFit("MultiRespFit", main_input=resp_mol2_high, cwd=self.cwd / "gaussianCalcs",
                              in_gaussian_label=rotation_label, out_respfit=out_respfit, **self.kwargs),
            StageUpdateCharge("UpdateCharge", main_input=resp_mol2_high, cwd=self.cwd, out_mol2=resp_mol2,
                              charge_column=3, charge_source=out_respfit, **self.kwargs),
            StageNormalizeCharge("Normalize2", main_input=resp_mol2, cwd=self.cwd, net_charge=self.net_charge,
                                 out_mol2=resp_mol2, **self.kwargs),
            StageUpdate("UpdateNames", main_input=resp_mol2, cwd=self.cwd, source_mol2=initial_mol2, out_mol2=resp_mol2,
                        update_names=True, update_types=False, update_resname=True, **self.kwargs),
            StageUpdate("UpdateTypes", main_input=resp_mol2, cwd=self.cwd, source_mol2=initial_mol2,
                        out_mol2=final_mol2, update_names=False, update_types=True, **self.kwargs),
